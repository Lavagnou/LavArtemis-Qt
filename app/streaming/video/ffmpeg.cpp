...
bool FFmpegVideoDecoder::createFrontendRenderer(PDECODER_PARAMETERS params, bool useAlternateFrontend)
{
    ...
    // LavArtemis: honour the user's explicit frontend renderer preference before
    // running the automatic selection below. RB_AUTO keeps upstream behaviour.
    ...
    if (useAlternateFrontend && m_BackendRenderer->getRendererType() != IFFmpegRenderer::RendererType::Vulkan) {
        ...
        if (preferredBackend == StreamingPreferences::RB_VULKAN) {
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                        "Renderer preference: trying Vulkan (PlVkRenderer) frontend first");
        ...
        if (preferredBackend == StreamingPreferences::RB_OPENGL && m_BackendRenderer->canExportEGL()) {
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                        "Renderer preference: trying OpenGL (EGLRenderer) frontend first");
