...
Session::~Session()
{
    ...
    // These two are owned by the session.
    ...
    // NB: m_ClipboardManager is NOT deleted here. It is a process-wide singleton
    // (ClipboardManager::instance()) whose s_instance is never cleared, so deleting
    // it would leave the next session holding a dangling pointer.
...
bool Session::initialize(QQuickWindow* qtWindow)
{
    ...
    // NB: The launch warning display loop that used to live here is gone. Upstream
    // now surfaces warnings through the launchWarnings Q_PROPERTY + launchWarningsChanged
    // signal, which QML renders; the old displayLaunchWarning signal no longer exists.
